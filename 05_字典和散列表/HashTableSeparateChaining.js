...
class HashTableSeparateChaining {
  ...
  put(key, value) {
    if (key != null && value != null) {
      ...
      if (this.table[position] == null) {
        this.table[position] = new LinkedList()     // 在该位置创建一个链表
      }
      this.table[position].push(new ValuePair(key, value))  //把值添加到在这个链表上
      return true
    }
    return false
  }
  get(key) {
    ...
    if (linkedList != null && linkedList.isEmpty()) {
      ...
      while (current != null) {               //迭代链表
        if (current.element.key === key) {    //根据key值来搜索
          return current.element.value        //若有有全等的key则返回链表上该元素
        }
        current = current.next                //若无则往下迭代直到结束
      }
    }
    return undefinedS
  }
  remove(key) {
    ...
    if (linkedList != null && linkedList.isEmpty()) {
      let current = linkedList.getHead()
      while (curren != null) {                //迭代链表
        if (current.element.key === key) {    //根据链表元素的key值来判断
          linkedList.remove(current.element)  //若有 则移除该链表上该元素
          if (linkedList.isEmpty()) {         //移除后判断该链表是否为空
            delete this.table[position]       //链表为空说明hash表该位置也为空，则删除
          }
          return true
        }
        current = current.next                //若无对应key值继续迭代至找到或者结束
      }
    }
    return false
  }
